<template lang>
  <div class="space-y-6">
    ...
    <SectionsPage :active-tab="this.selectedTab" section="main">
      ...
      >
        <template #view>
          ...
          <FieldPage title="Is Payment">
            <span
              :class="
                this.modelValue.order.is_payment == 'Y'
                  ? 'text-green-600'
                  : 'text-red-600'
              "
          ...
          <FieldPage
            @click="this.onCopy(this.modelValue.payments[0]?.number)"
            title="Payment ID"
        ...
        <template #edit>
          <SelectPage
            label="Service"
            v-model="this.modelValue.package.id"
            :options="this.packages"
            :get-option-label="
              (option) => option.name + ' ' + (option.last_name ?? '')
            "
      ...
      >
        <template #view>
          <FieldPage title="Inspection Date">
            {{
              this.modelValue.inspection_date
                ? new Date(this.modelValue.inspection_date).toLocaleString(
                    "en-US",
                    {},
                  )
                : "NONE"
          ...
          <FieldPage title="Inspector">
            <!-- INSPECTOR -->
            <router-link
              :to="`/h-admin/user/${this.modelValue.responsible?.id}`"
          ...
          <FieldPage title="Client">
            <router-link
              :to="`/h-admin/client/${this.modelValue.client_id}`"
              class="hover:text-blue-600"
            >
              <div>
                {{
          ...
          <FieldPage title="Seller">
            <div>
              {{ this.modelValue.seller_name }}
            </div>
            <div class="text-xs text-gray-400 mt-1">
              {{ "TEL: " + (this.modelValue.seller_phone ?? "") }}
            </div>
          </FieldPage>
          <FieldPage title="Vehicle Address">
            {{ this.modelValue.vehicle_address }}
          </FieldPage>
          <FieldPage title="Dealer">
            <div>
              {{ this.modelValue.dealer }}
            </div>
            <div
              v-if="this.modelValue.stock_number"
              class="text-xs text-gray-400 mt-1"
            >
              {{ "SN: " + (this.modelValue.stock_number ?? "") }}
            </div>
          </FieldPage>
        </template>
        ...
        <template #edit>
          <InputDatePage
            ...
            v-model="this.modelValue.inspection_date"
            @update:model-value="this.getUsers()"
          />

          <SelectPage
            label="Inspector"
            v-model="this.modelValue.responsible.id"
            :options="this.users"
            :get-option-label="
              (option) =>
          ...
          <SelectPage
            label="Client"
            v-model="this.modelValue.client_id"
          ...
          <InputTextPage
            v-model="this.modelValue.seller_name"
            title="Seller Name"
          />
          <InputTextPage
            v-model="this.modelValue.seller_phone"
            title="Seller Phone"
          />
          <InputTextPage
            v-model="this.modelValue.vehicle_address"
            title="Vehicle Address"
          />
          <InputTextPage v-model="this.modelValue.dealer" title="Dealer" />
          <InputTextPage
            v-model="this.modelValue.stock_number"
            title="Stock Number"
          />
        </template>
      </SectionPage>
      ...
      <SectionPage title="Documents" icon="fa fa-file-alt">
        ...
        <template #view>
          <div
            v-if="this.modelValue.documents"
            v-for="(document, index) in this.modelValue.documents"
            :key="index"
          >
            <div
            ...
            >
              <div class="flex items-center gap-3">
                <div
                  class="w-10 h-10 rounded-full bg-green-900/30 flex items-center justify-center text-green-400"
                >
              ...
              <div class="flex items-center gap-3">
                ...
                <button
                  @click="this.deleteDocument(index)"
    ...
    <SectionsPage :active-tab="this.selectedTab" section="payments">
      <SectionPage title="Payments" icon="fa fa-credit-card">
        ...
        <template #view>
          <FieldPage
            v-for="(payment, index) in this.modelValue.payments"
            :key="index"
          >
            <div class="flex items-center gap-4">
              <router-link
                :to="`/h-admin/payment/${payment.id}`"
                class="hover:text-blue-600"
              >
                Payment #{{ payment.id }}
                <span
                  :class="
                    payment.is_payment == 'Y'
                      ? 'text-green-600'
                      : 'text-red-600'
                  "
                  class="text-gray-400 mt-1"
              ...
              <div class="flex gap-2">
                <button
                  :href="payment.number"
                  class="cursor-pointer container mx-auto h-full"
                  target="_blank"
                  @click="this.copyNumber(payment)"
                >
                  <i
                    v-if="!payment.copiedNum"
                    class="fa fa-copy text-gray-500 hover:text-blue-500"
                  ></i>
                  <i
                    v-if="payment.copiedNum"
                ...
                <a
                  :href="payment.link"
                  class="text-blue-500 block text-center h-full"
                  target="_blank"
                >
                ...
                <button
                  ...
                  class="cursor-pointer container mx-auto h-full"
                  target="_blank"
                  @click="this.paymentDelete(payment)"
                >
                  <i
                    v-if="!payment.copiedNum"
                    class="fa fa-trash text-gray-500 hover:text-red-500"
                  ></i>
                </button>
              </div>
            </div>
    ...
    <SectionsPage :active-tab="this.selectedTab" section="addons">
      <SectionPage title="Addons" icon="fa fa-puzzle-piece">
        ...
        <template #view>
          <FieldPage
            v-for="(addon, index) in this.modelValue.addons"
            :key="index"
          >
            <div class="flex items-center gap-4">
              <div>{{ addon.name }}</div>
...
export default {
  ...
  methods: {
    ...
    async getUsers() {
      ...
      let query = `?inspection_date=${inspectionDate}&roles=${roles}&active=Y`;
      await axios.get("/users" + (query ? `${query}` : "")).then((response) => {
